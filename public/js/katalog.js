...
async function displayProducts(products = null) {
  try {
    ...
    const productsToDisplay = products || (await window.getAllUMKMAsync());
        ...
        const images =
          product.gambar_list && product.gambar_list.length > 0
            ...
            : product.gambar
              ...
              : [];
        const videos =
          product.video_list && product.video_list.length > 0
            ? product.video_list
            : [];
        return `
...
`;
    ...
    productsToDisplay.forEach((product) => {
      try {
        ...
        console.error(
          "[v0] Error initializing slider for product:",
          product.id,
          sliderError,
        );
      }
...
async function filterProducts() {
  try {
    ...
    const allProducts = await window.getAllUMKMAsync();
    ...
    const filtered = allProducts.filter((product) => {
      const matchesSearch =
        ...
        (product.deskripsi &&
          product.deskripsi.toLowerCase().includes(searchInput)) ||
        (product.alamat &&
          product.alamat.toLowerCase().includes(searchInput)) ||
        (product.lokasi && product.lokasi.toLowerCase().includes(searchInput));

      const matchesCategory =
        !categoryFilter ||
        product.kategori === categoryFilter ||
        product.jenis === categoryFilter;
...
function openAddModal() {
  ...
  setTimeout(() => {
    if (!mapPickerAdd) {
      mapPickerAdd = window.L.map("mapPickerUMKMAdd").setView(
        [-7.4186, 110.9758],
        14,
      );
...
function previewVideos(event) {
  ...
  Array.from(files).forEach((file, index) => {
    ...
    if (file.size > 50 * 1024 * 1024) {
      console.warn(
        `[v0] Video ${index + 1} terlalu besar: ${sizeMB.toFixed(2)}MB`,
      );
      alert(`Video ${index + 1} terlalu besar! Maksimal 50MB per video.`);
      return;
    }
    console.log(
      `[v0] Video ${index + 1} valid: ${sizeMB.toFixed(2)}MB, type: ${file.type}`,
    );
  ...
  // Add new files to the array
  console.log(
    `[v0] Adding ${validFiles.length} valid videos to selectedProductVideos`,
  );
  selectedProductVideos = [...selectedProductVideos, ...validFiles];
  console.log(
    `[v0] selectedProductVideos length now: ${selectedProductVideos.length}`,
  );
...
function renderVideoPreviews(container, filesArray, inputId) {
  ...
  filesArray.forEach((file, index) => {
    ...
    reader.onerror = () => {
      clearTimeout(timeout);
      console.error(
        `[v0] Error reading video file ${index + 1}:`,
        reader.error,
      );
    ...
    console.log(
      `[v0] Start reading video ${index + 1}, size: ${(file.size / 1024 / 1024).toFixed(2)}MB`,
    );
    reader.readAsDataURL(file);
  });
}
...
function previewEditVideos(event) {
  try {
    ...
    console.log(
      `[v0] Adding ${validFiles.length} valid videos to selectedEditProductVideos`,
    );
    selectedEditProductVideos = [...selectedEditProductVideos, ...validFiles];
    console.log(
      `[v0] selectedEditProductVideos length now: ${selectedEditProductVideos.length}`,
    );
    ...
    const videoPromises = selectedProductVideos.map((file, fileIndex) => {
      return new Promise((resolve) => {
        ...
        if (file.size > 20 * 1024 * 1024) {
          console.warn(
            `[v0] Video ${fileIndex + 1} terlalu besar: ${fileSizeMB.toFixed(2)}MB`,
          );
          alert(
            `Video ${fileIndex + 1} terlalu besar! Maksimal 20MB per video untuk performa optimal.`,
          );
          resolve(null);
          return;
        }
        ...
        const timeout = setTimeout(() => {
          console.error(
            `[v0] Video ${fileIndex + 1} timeout saat membaca file (>120s)`,
          );
          reader.abort();
          alert(
            `Video ${fileIndex + 1} timeout. File terlalu besar atau koneksi lambat.`,
          );
          resolve(null);
        ...
        reader.onload = (e) => {
          ...
          console.log(
            `[v0] Video ${fileIndex + 1} berhasil dikonversi, ukuran base64: ${base64Size}MB`,
          );

          // Warn if base64 is too large for localStorage
          if (e.target.result.length > 4 * 1024 * 1024) {
            console.warn(
              `[v0] Video ${fileIndex + 1} base64 size ${base64Size}MB mungkin terlalu besar untuk localStorage`,
            );
          }
        ...
        reader.onerror = () => {
          clearTimeout(timeout);
          console.error(
            `[v0] Error membaca video file ${fileIndex + 1}:`,
            reader.error,
          );
          alert(
            `Gagal membaca video ${fileIndex + 1}: ${reader.error || "Unknown error"}`,
          );
          resolve(null);
        };
        reader.onabort = () => {
        ...
        console.log(
          `[v0] Mulai membaca video ${fileIndex + 1}, ukuran: ${fileSizeMB.toFixed(2)}MB`,
        );
        reader.readAsDataURL(file);
      });
    });
  ...
  Promise.all(promises)
    .then(([images, videos]) => {
      try {
        ...
        if (selectedProductVideos.length > 0 && validVideos.length === 0) {
          ...
          alert(
            "Gagal memproses video. Pastikan file tidak terlalu besar dan format didukung.",
          );
          return;
        }

        console.log("[v0] Add product data:", {
        ...
        if (
          ...
          alert(
            "Data terlalu besar! Gunakan video yang lebih kecil atau kompres terlebih dahulu.",
          );
        } else {
          alert(
            `Terjadi error saat menyimpan: ${error.message || "Unknown error"}`,
          );
        }
      }
    })
    .catch((error) => {
      ...
      if (
        ...
        alert(
          `Terjadi error saat memproses file: ${error.message || "Unknown error"}`,
        );
      }
    });
}
...
function openEditModal(id) {
  ...
  setTimeout(() => {
    if (!mapPickerEdit) {
      mapPickerEdit = window.L.map("mapPickerUMKMEdit").setView(
        [product.lat || -7.4186, product.lng || 110.9758],
        14,
      );
      window.L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        attribution: "© OpenStreetMap contributors",
      ...
      mapPickerEdit.on("click", (e) => {
        ...
        document.getElementById("editProductLat").value =
          e.latlng.lat.toFixed(6);
        document.getElementById("editProductLng").value =
          e.latlng.lng.toFixed(6);
      });
    } else {
      mapPickerEdit.setView(
        [product.lat || -7.4186, product.lng || 110.9758],
        14,
      );
...
function updateProduct(event) {
  ...
  Promise.all(promises)
    .then(([newImages, newVideos]) => {
      try {
        // Filter out null values from failed conversions
        const validNewVideos = newVideos.filter((v) => v !== null);

        if (
          selectedEditProductVideos.length > 0 &&
          validNewVideos.length === 0
        ) {
          console.warn("[v0] Semua video gagal dikonversi");
          alert(
            "Gagal memproses video. Pastikan file tidak terlalu besar (max 50MB per video)",
          );
          return;
        }
        ...
        const finalImages = [
          ...(window.existingProductImages || []).filter(
            (img) => !deletedExistingProductImages.includes(img),
          ),
          ...newImages,
        ].filter((v) => v);

        const finalVideos = [
          ...(window.existingProductVideos || []).filter(
            (vid) => !deletedExistingProductVideos.includes(vid),
          ),
          ...validNewVideos,
        ].filter((v) => v);
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  window.addEventListener("umkmDataChanged", async () => {
    await displayProducts();
  });
});
...
function renderExistingMediaPreviews() {
  try {
    ...
    if (
      ...
      window.existingProductImages.length > 0
    ) {
      ...
      selectedEditProductImages.forEach((file, index) => {
        ...
        reader.onload = (e) => {
          try {
            ...
            if (
              index < selectedEditProductImages.length &&
              selectedEditProductImages[index] === file
            ) {
              const imgContainer = document.createElement("div");
              imgContainer.style.position = "relative";
              imgContainer.style.display = "inline-block";
    ...
    if (
      window.existingProductVideos &&
      window.existingProductVideos.length > 0
    ) {
      const header = document.createElement("div");
      header.innerHTML =
        '<p style="font-size: 12px; color: #64748b; font-weight: 600; margin-bottom: 8px; width: 100%;">Video yang ada:</p>';
    ...
    if (selectedEditProductVideos.length > 0) {
      const newHeader = document.createElement("div");
      newHeader.innerHTML =
        '<p style="font-size: 12px; color: #059669; font-weight: 600; margin-bottom: 8px; margin-top: 16px; width: 100%;">Video baru:</p>';
      ...
      selectedEditProductVideos.forEach((file, index) => {
        ...
        reader.onload = (e) => {
          try {
            ...
            if (
              index < selectedEditProductVideos.length &&
              selectedEditProductVideos[index] === file
            ) {
              const videoContainer = document.createElement("div");
              videoContainer.style.position = "relative";
              videoContainer.style.display = "inline-block";

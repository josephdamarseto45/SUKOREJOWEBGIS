...
async function displayWisata(wisataList = null) {
  try {
    ...
    const wisataToDisplay = wisataList || (await window.getAllWisataAsync());
        ...
        const images =
          wisata.foto_list && wisata.foto_list.length > 0
            ...
            : wisata.foto
              ...
              : [];
        const videos =
          wisata.video_list && wisata.video_list.length > 0
            ? wisata.video_list
            : [];
        return `
        ...
        if (videos.length > 0) {
          ...
          if (videoContainer && window.ImageSlider) {
            ...
            new window.ImageSlider(
              videoContainer,
              videoMedia,
              `wisata-video-${wisata.id}`,
            );
          }
        }
      } catch (sliderError) {
        console.error(
          "[v0] Error initializing slider for wisata:",
          wisata.id,
          sliderError,
        );
      }
...
async function filterWisata() {
  try {
    const searchInput =
      document.getElementById("searchInput")?.value?.toLowerCase() || "";
    const categoryFilter =
      document.getElementById("categoryFilter")?.value || "";
    const allWisata = await window.getAllWisataAsync();
    ...
    const filtered = allWisata.filter((wisata) => {
      const matchesSearch =
        ...
        (wisata.deskripsi &&
          wisata.deskripsi.toLowerCase().includes(searchInput)) ||
        (wisata.kategori &&
          wisata.kategori.toLowerCase().includes(searchInput));

      const matchesCategory =
        !categoryFilter || wisata.kategori === categoryFilter;

      return matchesSearch && matchesCategory;
    });

    await displayWisata(filtered);
  } catch (error) {
...
function openDetailModal(id) {
  ...
  if (media.length > 0) {
    const galleryContainer = document.getElementById(
      `wisata-detail-gallery-${wisata.id}`,
    );
    if (galleryContainer) {
...
function openAddWisataModal() {
  ...
  setTimeout(() => {
    if (!mapPickerAdd) {
      mapPickerAdd = window.L.map("mapPickerWisataAdd").setView(
        [-7.5626, 110.8282],
        13,
      );
...
function removeWisataImage(index, inputId) {
  if (inputId === "wisataImage") {
    ...
    renderWisataImagePreviews(
      document.getElementById("wisataImagePreview"),
      selectedWisataImages,
      "wisataImage",
    );
...
function removeWisataVideo(index, inputId) {
  if (inputId === "wisataVideo") {
    ...
    renderWisataVideoPreviews(
      document.getElementById("wisataVideoPreview"),
      selectedWisataVideos,
      "wisataVideo",
    );
...
function openEditWisataModal(id) {
  ...
  document.getElementById("editWisataJamOperasional").value =
    wisata.jamOperasional || "";
  document.getElementById("editWisataHargaTiket").value =
    wisata.hargaTiket || "";
  ...
  setTimeout(() => {
    if (!mapPickerWisataEdit) {
      mapPickerWisataEdit = window.L.map("mapPickerWisataEdit").setView(
        [wisata.lat || -7.5626, wisata.lng || 110.8282],
        14,
      );
      window.L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        attribution: "© OpenStreetMap contributors",
      ...
      mapPickerWisataEdit.on("click", (e) => {
        ...
        document.getElementById("editWisataLat").value =
          e.latlng.lat.toFixed(6);
        document.getElementById("editWisataLng").value =
          e.latlng.lng.toFixed(6);
      });
    } else {
      mapPickerWisataEdit.setView(
        [wisata.lat || -7.5626, wisata.lng || 110.8282],
        14,
      );
...
function updateWisata(event) {
  ...
  const jamOperasional = document.getElementById(
    "editWisataJamOperasional",
  ).value;
  ...
  if (selectedEditWisataVideos.length > 0) {
    const videoPromises = selectedEditWisataVideos.map((file, fileIndex) => {
      return new Promise((resolve) => {
        ...
        if (file.size > 50 * 1024 * 1024) {
          console.warn(
            `[v0] Video ${fileIndex + 1} terlalu besar: ${(file.size / 1024 / 1024).toFixed(2)}MB`,
          );
          resolve(null);
          return;
        }
        ...
        const timeout = setTimeout(() => {
          console.error(
            `[v0] Video ${fileIndex + 1} timeout saat membaca file`,
          );
          reader.abort();
          resolve(null);
        ...
        reader.onload = (e) => {
          ...
          console.log(
            `[v0] Video ${fileIndex + 1} berhasil dikonversi, ukuran base64: ${(e.target.result.length / 1024 / 1024).toFixed(2)}MB`,
          );
        ...
        console.log(
          `[v0] Mulai membaca video ${fileIndex + 1}, ukuran: ${(file.size / 1024 / 1024).toFixed(2)}MB`,
        );
        reader.readAsDataURL(file);
      });
    });
  ...
  Promise.all(promises)
    .then(([newImages, newVideos]) => {
      try {
        ...
        if (
          selectedEditWisataVideos.length > 0 &&
        ...
        console.log("[v0] Final wisata data:", {
...
function renderExistingWisataMediaPreviews() {
  try {
    ...
    if (window.existingWisataImages && window.existingWisataImages.length > 0) {
      ...
      header.innerHTML =
        '<p style="font-size: 12px; color: #64748b; font-weight: 600; margin-bottom: 8px; width: 100%;">Foto yang ada:</p>';
    ...
    if (selectedEditWisataImages.length > 0) {
      ...
      newHeader.innerHTML =
        '<p style="font-size: 12px; color: #7fa998; font-weight: 600; margin-bottom: 8px; margin-top: 16px; width: 100%;">Foto baru:</p>';
      ...
      selectedEditWisataImages.forEach((file, index) => {
        ...
        reader.onload = (e) => {
          try {
            ...
            if (
              index < selectedEditWisataImages.length &&
              selectedEditWisataImages[index] === file
            ) {
              const imgContainer = document.createElement("div");
              imgContainer.style.position = "relative";
              imgContainer.style.display = "inline-block";
    ...
    const existingVideosContainer = document.getElementById(
      "editWisataVideoPreview",
    );
    ...
    if (selectedEditWisataVideos.length > 0) {
      const newHeader = document.createElement("div");
      newHeader.innerHTML =
        '<p style="font-size: 12px; color: #7fa998; font-weight: 600; margin-bottom: 8px; margin-top: 16px; width: 100%;">Video baru:</p>';
      ...
      selectedEditWisataVideos.forEach((file, index) => {
        ...
        reader.onload = (e) => {
          try {
            ...
            if (
              index < selectedEditWisataVideos.length &&
              selectedEditWisataVideos[index] === file
            ) {
              const videoContainer = document.createElement("div");
              videoContainer.style.position = "relative";
              videoContainer.style.display = "inline-block";

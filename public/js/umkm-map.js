...
async function initializeUMKMMap() {
  // Initialize main map
  ...
  // Load all UMKM data from API (fresh data, not cache)
  allUMKMData = await window.getAllUMKMAsync();
...
function displayUMKMMarkers(data) {
  ...
  data.forEach((umkm) => {
    ...
    const marker = window.L.marker([umkm.lat, umkm.lng], { icon }).addTo(
      umkmMap,
    );
